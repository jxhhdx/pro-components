<template>
  ...
  >
    ...
    <a-menu-item-group v-if="false" title="高级组件">
    ...
    watch(
      ...
      () => {
        ...
        let title = props.isZhCN
          ? 'ProComponents - 一套企业级 Vue 组件库'
          : 'ProComponents — An enterprise-class UI components based on Ant Design and Vue.js';
        if (item && item.title) {
          title = props.isZhCN
            ? `${item.subtitle || ''} ${item.title} - ProComponents`
            : `${item.enTitle || item.title} - ProComponents`;
        }
